...
- 在网页中快速配置模型，包括 `apiKey`、`baseUrl`、`provider`、`model` 和能力类型。
...
- [ ] 模型配置管理页面。
- [ ] 模型配置保存到 PostgreSQL。
- [ ] API key 只在后端保存，前端只展示脱敏值。
- [ ] 题集列表页面。
- [ ] 从服务器本地目录导入 `custom_medical_eval_sets/*.jsonl`。
- [ ] 题目保存原始 JSON、题干、选项、标准答案和题型。
- [ ] 评测运行页面，可选择一个题集和一个或多个模型。
- [ ] 后端创建评测运行记录和逐题结果记录。
- [ ] 后端逐题调用模型并保存回答、耗时、错误和评分结果。
- [ ] 前端结果页展示运行状态、进度、准确率和逐题明细。
- [ ] 前端通过轮询实现第一版实时更新。
...
| 展示名称 | provider | 默认模型 | 说明 |
| --- | --- | --- | --- |
| AntAngelMed（安诊儿） | `ant_ling` | `AntAngelMed` | 医疗文本模型 |
| DeepSeek-v4-pro | `deepseek` | `DeepSeek-v4-pro` | OpenAI 兼容接口 |
| qwen3.7-plus | `qwen` | `qwen3.7-plus` | OpenAI 兼容接口 |
| gpt-5.5 | `openai_responses` | `gpt-5.5` | OpenAI Responses API |
| Gemini-3.5-flash | `gemini` | `Gemini-3.5-flash` | Google Gemini |
...
| 展示名称 | provider | 默认模型 |
| --- | --- | --- |
| qwen3.7-plus | `qwen_vision` | `qwen3.7-plus` |
| gpt-5.5 | `openai_responses` | `gpt-5.5` |
...
| 文件 | 题量 | 类型 | 字段 |
| --- | ---: | --- | --- |
| `dataset_upload_prod_医疗文本推理_0_59300178.jsonl` | 999 | 选择题 | `question`、`options`、`answer`、`cot` |
| `dataset_upload_prod_医疗安全_0_59300178.jsonl` | 299 | 问答题 | `question`、`answer` |
| `dataset_upload_prod_医疗伦理_0_59300178.jsonl` | 299 | 问答题 | `question`、`answer` |
...
- `apiKey` 不返回明文。
- provider 适配层参考已验证的 provider 形态：
...
- 题集管理：题集列表、导入内置 `custom_medical_eval_sets`、查看题目。
- 评测运行：选择题集和模型，发起评测。
- 结果页面：运行列表、进度、准确率、逐题结果、错误信息。

## 验收标准

- [ ] H 场景：用户可以在网页创建一个文本模型配置，保存后列表可见，API key 只显示脱敏值。
- [ ] H 场景：用户可以一键导入 `custom_medical_eval_sets`，导入后看到 3 个题集和对应题量。
- [ ] H 场景：用户可以选择一个题集和多个模型发起评测。
- [ ] H 场景：评测运行时结果页面持续更新进度。
- [ ] H 场景：选择题能够抽取模型答案并计算正确率。
- [ ] H 场景：问答题能够保存模型回答、标准答案和评分状态。
- [ ] A 场景：某个模型调用失败时，该模型对应结果记录错误，其他模型和题目继续执行。
- [ ] A 场景：重复导入同一 JSONL 文件不会造成题目无限重复。
- [ ] S 场景：前端任何接口都不会返回 API key 明文。
- [ ] S 场景：后端运行依赖 PostgreSQL，Docker 本地和测试环境均配置 PostgreSQL。
